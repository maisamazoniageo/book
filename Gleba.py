...
for key, value in estados.items():
    sigef_geral[sigef_geral["uf_sigla"] == key].to_file(
        "../glebas-federais.gpkg", layer=f"sigef-{value}"
    )

for key, value in estados.items():
    snci_geral[snci_geral["uf_municip"] == key].to_file(
        "../glebas-federais.gpkg", layer=f"snci-{value}"
    )
# %%
